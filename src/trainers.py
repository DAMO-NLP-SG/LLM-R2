...
class CLTrainer(Trainer):
    # todo: for evaluation we just need to change this evaluate STSB data into ours like sentence pair with 0 / 5 scores
    def evaluate(
            self,
        ...
        def batcher(params, batch):
            ...
            def conv_dict(in_dict):
                ...
                for key in list(in_dict.keys()):
                    if key == 'heights' or key == 'adjacency_list' or key == 'rel_pos':
                        in_dict[key] = torch.LongTensor(ast.literal_eval(in_dict[key].replace('-inf', '-2e308')))
                    else:
                        in_dict[key] = torch.FloatTensor(ast.literal_eval(in_dict[key].replace('-inf', '-2e308')))
                return in_dict

            sentences = [[conv_dict(ast.literal_eval(' '.join([x.replace('-inf', '-2e308') for x in s]).replace('"', '').strip()))] for s in batch]
            # print(len(sentences))
            # print(len(sentences[0]))
            # print(type(sentences[0]))
            # sent_features = prepare_enc_data(sentences, db_ids)
            batch = eval_collator(sentences)
            with torch.no_grad():
